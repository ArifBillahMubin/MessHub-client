...
const FindMess = () => {
    ...
    // Marker click scrolls to the card (unchanged)
    ...
    // Quick Locations — derived from actual results
    const quickLocations = [...new Set(
        posts.flatMap(p => [p.mess?.location?.area, p.mess?.location?.city].filter(Boolean))
    )].slice(0, 6);

    // Nearby count — messes within 2 km of user (client-side only)
    const nearbyCount = userLocation
        ? posts.filter(p => {
            const lat = p.mess?.location?.latitude;
            const lng = p.mess?.location?.longitude;
            return isNearby(userLocation.lat, userLocation.lng, lat, lng);
          }).length
        : 0;
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="bg-gradient-to-b from-background to-white border-b border-primary/10">
                <div className="mx-auto max-w-7xl px-4 pt-7 pb-6 sm:px-6 lg:px-8">
                    ...
                    <div className="flex flex-col gap-6 lg:flex-row lg:items-stretch">
                        ...
                        {/* LEFT: breadcrumb + headline + search + filters */}
                        <div className="flex-1 min-w-0">
                            {/* Breadcrumb */}
                            <nav className="mb-4 flex items-center gap-1.5 text-xs font-medium text-neutral/50">
                                ...
                                {(q || messType) && (
                                    <>
                                        ...
                                        <span className="text-primary font-semibold capitalize">
                                            {q || MESS_TYPES.find(t => t.value === messType)?.label}
                                        </span>
                                    </>
                                )}
                            </nav>
                            ...
                            <p className="mt-3 max-w-xl text-base text-neutral/60 leading-relaxed">
                            ...
                            {/* Search */}
                            <div className="mt-5">
                                <div className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-white px-5 py-3.5 shadow-sm transition focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/10">
                            ...
                            {/* More Filters panel (logic unchanged) */}
                            {moreOpen && (
                            ...
                            <div className="h-full min-h-[280px] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm lg:min-h-0">
                                ...
                                <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-100">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {!userLocation && !locationError && (
                                            <span className="text-[10px] text-neutral/40">Enable location</span>
                                        )}
                                ...
                                {quickLocations.length > 0 && (
                                    <div className="p-3 border-t border-gray-100">
                                        <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-neutral/40">Quick Locations</p>
                                        <div className="flex flex-wrap gap-1.5">
            ...
            <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                ...
                {/* 4-column card grid (desktop), 2-column (tablet), 1-column (mobile) */}
                {!isLoading && !isError && posts.length > 0 && (
